...
def init_args():
    ...
    parser.add_argument('--config_file', default='/root/DBNet.pytorch/config/icdar2015_resnext_FPN_DBhead_polyLR.yaml', type=str)
...
def main(config):
    ...
    if torch.cuda.device_count() > 1:
        ...
        torch.distributed.init_process_group(
            backend="nccl", init_method="env://", world_size=torch.cuda.device_count(), rank=args.local_rank)
    ...
    train_loader = get_dataloader(
        config['dataset']['train'], config['distributed'])
